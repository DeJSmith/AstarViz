    ...
    this.build = function()
    {
        ...
        for (var i = 0; i < this.cols; i++) {
            ...
            for (var j = 0; j < this.rows; j++) {
                node = new Node(i, j, x + i * w /this.cols, y + j * h / this.rows, w / this.cols, h / this.rows, true, this.grid);
                this.grid[i][j] = node;
...
function BspRoom(container)
{
    ...
    this.decorate_circle = function(grid)
    {
       ...
       while (x >= y)
       {
           grid[x0 + x][y0 + y].obstacle = true;
           grid[x0 + y][y0 + x].obstacle = true;
           grid[x0 - y][y0 + x].obstacle = true;
           grid[x0 - x][y0 + y].obstacle = true;
           grid[x0 - x][y0 - y].obstacle = true;
           grid[x0 - y][y0 - x].obstacle = true;
